...
class InternalDocumentNumber
{
    ...
    public function nextYearly(string $prefix, int $padding = 4): string
    {
        $year = now()->format('Y');
        $sequenceKey = $prefix . ':' . $year;
        $existingValue = $this->existingYearlyValue($prefix, $year);
    ...
    private function existingYearlyValue(string $prefix, string $year): int
    {
        $sources = [
            'BTP' => ['internal_btp_production_orders', 'btp_order_code'],
        ];
