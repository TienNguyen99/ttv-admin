...
class InternalInventoryReportController extends Controller
{
    ...
    private function exportCurrentStock(
        ...
        $movementByCode = app(InternalStockLedger::class)
            ...
            ->map(function ($row) use ($catalogMap, $resolver) {
                ...
                return [
                    ...
                    'unit' => mb_strtoupper(trim((string) ($catalog['unit'] ?? ''))) ?: 'CHƯA CÓ ĐVT',
        ...
        $currentStockRows = app(InternalStockLedger::class)
            ->query($monthStart, $monthEnd)
            ->select(
                'location_code',
                'internal_item_code',
            ...
            ->map(function ($row) use ($catalogMap, $resolver, $movementByCode) {
                ...
                return [
                    ...
                    'name' => $name,
                    'group' => $catalog['group'] ?? $resolver->resolveName($name),
                    'unit' => mb_strtoupper(trim((string) ($catalog['unit'] ?? ($movement['unit'] ?? '')))) ?: 'CHƯA CÓ ĐVT',
        ...
        foreach ($selectedGroups as $index => $group) {
            ...
            $sheet->setTitle($this->sheetTitle($group, $usedTitles));
            $this->writeCurrentStockSheet($sheet, $group, $asOf, $currentStockRows->get($group, collect()));
        }
        if ($selectedGroups->isEmpty()) {
