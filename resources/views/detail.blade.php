...
<body>
    <h1>CHI TIẾT LỆNH SỐ: {{ $lenh[0]->So_dh ?? 'Không có dữ liệu' }} - {{ $lenh[0]->Ten_hh ?? 'Không có dữ liệu' }}
    </h1>
        ...
        <tbody>
            @foreach ($lenh as $item)
                <tr>
                    ...
                    <td>{{ $item->Ngay2ct ? \Carbon\Carbon::parse($item->Ngay2ct)->format('d/m/Y') : 'Chưa nhập kho' }}
                    </td>
                    <td>
